...
client = Client(Environment.PRODUCTION, 360)  # 1 is the test app index

sourcePri = ''
# source = PrivateKey.from_base58(sourcePri)
source = PrivateKey.from_string(sourcePri)
print(source)

destinationPub = 'CvHoY9hk8LbUhtqmr1rnh2gEUpsfUrQFvJdmGfFg4e5H'
dest = PublicKey.from_base58(destinationPub)
print(dest)
# dest = PublicKey.from_string(destinationPub)

# Send a payment of 1 Kin
payment = Payment(source, dest, TransactionType.EARN, kin_to_quarks('1'))
# try:
#     tx_id = client.submit_payment(payment)
#     print(
